...
export async function POST(request) {
  try {
    ...
    const [updatedUser, updatedViolation, transaction] = await prisma.$transaction([
      ...
      prisma.transaction.create({
        data: {
          ...
          type: 'debit',
          description: `Speed violation fine — ${violation.speed} km/h on ${new Date(violation.timestamp).toLocaleDateString('en-IN')}`,
        }
      })
    ])
    ...
    console.error('Wallet pay-fine error:', error)
